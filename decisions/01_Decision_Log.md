...
<!-- canonical-map:srv-noctua-linux-dedicated-campaign:D-accepted -->

### D-029 — Acceptance of validated srv-noctua Linux dedicated campaign evidence
- **Status:** Frozen
- **Date:** 2026-05-08
- **Decision:** The completed `srv-noctua` Linux dedicated execution of `EXP-MULTILEVEL-EXCEPTION-MINING-001` is accepted as validated environment-specific evidence for the explicit stratum `srv_noctua_linux_8gb`. The accepted evidence consists of `22400` planned runs, `22400` raw results, `22400` valid results, `0` invalid results, `0` missing artifacts, and `0` schema errors at repository head `354447be68b5f7361afd245897d91bea7329020f`.
- **Rationale:** The final validation gate reported `exit_status = 0`, `22400` `confirmation_row.json` artifacts, `planned_run_count = raw_result_count = valid_result_count = 22400`, `invalid_result_count = 0`, `missing_artifact_count = 0`, and `schema_error_count_total = 0`. The compact evidence bundle was copied locally and verified by SHA256 checksums.
- **Impact:** These outputs may be used as evidence only for the declared environment stratum `srv_noctua_linux_8gb` until additional environment strata are validated. The result does not authorize hardware-independent timing claims, pooled WSL/server claims, Windows-host claims, or final monograph claims before the results are mapped into the canonical result-to-text layer.
