# ruff: noqa: E402
"""Validate GRASP-Rust-fidelity against the canonical Python GRASP implementation.
...
def _python_grasp_payload(case: ValidationCase) -> dict[str, Any]:
    ...
    result = run_grasp_partition(
        ...
        config=GRASPConfig(
            ...
            checkpoint_every_iter=case.checkpoint_every_iter,
        ),
    )
...
def _rust_grasp_payload(case: ValidationCase, work_root: Path) -> dict[str, Any]:
    instance_path = work_root / f"{case.name}.json.gz"
    out_json = work_root / f"{case.name}__grasp_rust.json"
    ...
    if part_path_value is None:
        raise KeyError("grasp_rust canonical payload is missing paths.part_path")

    labels = read_partition_labels(Path(part_path_value))
    cut_from_labels = compute_cutsize_edges_labels(edges, labels)

    raw_json_path = Path(str(payload["paths"]["workdir"])) / "grasp_rust_result.json"
    if not raw_json_path.exists():
        raise FileNotFoundError(f"GRASP-Rust raw JSON not found: {raw_json_path}")
    ...
    payload["raw_grasp_rust_json_path"] = str(raw_json_path)
    payload["raw_grasp_rust_nfe"] = int(raw_payload["nfe"])
    payload["raw_grasp_rust_cutsize_best"] = int(raw_payload["cutsize_best"])
    return payload


def validate_case(case: ValidationCase, work_root: Path) -> dict[str, Any]:
    """Validate one controlled case."""
    py = _python_grasp_payload(case)
    rs = _rust_grasp_payload(case, work_root)
    ...
    checks = {
        ...
        "rust_raw_json_exists": Path(str(rs["raw_grasp_rust_json_path"])).exists(),
        ...
        == rs["raw_grasp_rust_cutsize_best"],
        "rust_final_checkpoint_matches_nfe": rs["checkpoints"][-1]["nfe"]
        == rs["raw_grasp_rust_nfe"],
    ...
    return {
        ...
        "grasp_rust": rs,
        "checkpoint_details": {
            "python_grasp": py_checkpoints,
            "grasp_rust": rs_checkpoints,
        },
        "checks": checks,
...
def run_validation(output: Path) -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="grasp_rust_validation_") as tmp:
        work_root = Path(tmp)
        cases = [validate_case(case, work_root) for case in default_cases()]

    report = {
        "schema_version": "grasp-rust-validation-v1",
        "claim_boundary": (
            "This report validates artifact/semantic invariants for GRASP-Rust-fidelity. "
...
def main() -> None:
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("audit_reports/grasp_rust_validation/grasp_rust_validation_report.json"),
    )
