...
def test_enabled_supported_solvers_includes_ts_rust_when_enabled():
    plan = {
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
            ...
            "grasp": {"enabled": False},
            "ts": {"enabled": False},
            "ts_rust": {"enabled": True},
        }
    }

    assert _enabled_supported_solvers(plan) == ["ts_rust"]


@pytest.mark.skipif(shutil.which("cargo") is None, reason="cargo not installed")
def test_run_plan_ts_rust_writes_schema_compatible_output_and_manifest(tmp_path: Path):
    ...
    plan = {
        "schema": "forja-exp-v1",
        "experiment_id": "test-ts-rust-plan",
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
            "sa": {"enabled": False},
    ...
    out_json = raw_dir / "toy.json.gz__ts_rust__k2__b0.10__seed42.json"
