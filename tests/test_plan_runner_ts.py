...
def test_enabled_supported_solvers_includes_ts_when_enabled():
    plan = {
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
    ...
    assert _enabled_supported_solvers(plan) == ["ts"]


def test_run_plan_ts_writes_schema_compatible_output_and_manifest(tmp_path: Path):
    ...
    out_json = raw_dir / "toy.json.gz__ts__k2__b0.10__seed42.json"
