...
def _write_instance(tmp_path: Path) -> Path:
    inst = {
        "num_nodes": 4,
...
def test_runner_does_not_duplicate_legacy_top_level_path_and_time_fields(
    monkeypatch, tmp_path: Path, algo: str
):
    inst_path = _write_instance(tmp_path)
    ...
    assert "elapsed_wall_ms" not in data

    assert "paths" in data
    assert data["paths"]["workdir"] == str(workdir)
    assert data["paths"]["graph_path"] == str(workdir / "graph.graph")
    assert data["paths"]["part_path"] == str(part_path)

    assert "workdir" not in data
    assert "graph_path" not in data
    assert "part_path" not in data
