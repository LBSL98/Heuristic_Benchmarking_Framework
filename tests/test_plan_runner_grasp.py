...
def test_enabled_supported_solvers_includes_grasp_when_enabled():
    plan = {
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
    ...
    assert _enabled_supported_solvers(plan) == ["grasp"]


def test_run_plan_grasp_writes_schema_compatible_output_and_manifest(tmp_path: Path):
    ...
    out_json = raw_dir / "toy.json.gz__grasp__k2__b0.10__seed42.json"
