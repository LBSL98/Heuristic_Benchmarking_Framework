...
def test_run_plan_manifest_index_excludes_preexisting_root_jsons(monkeypatch, tmp_path: Path):
    calls = []
    ...
    raw_dir = tmp_path / "raw"
    raw_dir.mkdir()
    (raw_dir / "smoke_metis.json").write_text('{"algo":"metis"}', encoding="utf-8")
    ...
    plan = {
        "schema": "forja-exp-v1",
        "experiment_id": "test-plan-manifest-scope",
        "solvers": {
            "greedy": {"enabled": False},
            "metis": {"enabled": True, "k": 8, "budget": {"type": "time", "seconds": 5}},
            "kahip": {
    ...
    assert len(rows) == 4
    assert "smoke_metis.json" not in {Path(row["_file"]).name for row in rows}
    assert {row["_file"] for row in rows} == {str(call["out_json"]) for call in calls}
