...
def test_enabled_supported_solvers_includes_ils_rust_when_enabled():
    plan = {
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
    ...
    assert _enabled_supported_solvers(plan) == ["ils_rust"]


@pytest.mark.skipif(shutil.which("cargo") is None, reason="cargo not installed")
def test_run_plan_ils_rust_writes_schema_compatible_output_and_manifest(tmp_path: Path):
    ...
    plan = {
        "schema": "forja-exp-v1",
        "experiment_id": "test-ils-rust-plan",
        "solvers": {
            "metis": {"enabled": False},
            "kahip": {"enabled": False},
            "sa": {"enabled": False},
    ...
    out_json = raw_dir / "toy.json.gz__ils_rust__k2__b0.10__seed42.json"
    assert out_json.exists()

    data = json.loads(out_json.read_text(encoding="utf-8"))
    assert data["algo"] == "ils_rust"
